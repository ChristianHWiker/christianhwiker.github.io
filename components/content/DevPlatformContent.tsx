...
export default function DevPlatformContent() {
  const { lang } = useLanguage();
  const tr = t.devPlatform;
    ...
    <div className="container mx-auto max-w-4xl px-6 py-24">
      ...
      <FadeIn>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {["Azure DevOps", "Docker", "Linux", "CI/CD"].map((tag) => (
            <span
              key={tag}
      ...
      <FadeIn>
        <div className="mt-12 grid grid-cols-3 gap-6 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-8">
          <AnimatedCounter end={2} label={tr.counters.vms[lang]} />
          <AnimatedCounter end={2} label={tr.counters.approvals[lang]} />
          <AnimatedCounter end={1} label={tr.counters.standard[lang]} />
        </div>
      </FadeIn>
